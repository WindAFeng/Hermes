...
use crate::models::redis_model::redis_pattern::RedisPattern;
...
impl RedisExecute {
    ...
    pub async fn set(&mut self, key: &str, value: &str) -> Result<(), HermesError> {
        self.execute(RedisCommands::SET, RedisArgument::Default(key.to_string(), value.to_string())).await
    }
    pub async fn del(&mut self, key: String) -> Result<(), HermesError> {
        // 删除指定 key
        self.execute(RedisCommands::DEL, RedisArgument::OnlyKey(key))
            .await
    }
    pub async fn dump(&mut self, key: String) -> Result<String, HermesError> {
        // 返回 base64 格式的字符串
        let dump_bytes: Vec<u8> = match self
            .execute(RedisCommands::DUMP, RedisArgument::OnlyKey(key))
            .await
        {
            Ok(bytes) => bytes,
            Err(e) => return Err(e),
    ...
    pub async fn exists(&mut self, key: String) -> Result<bool, HermesError> {
        ...
        self.execute(RedisCommands::EXISTS, RedisArgument::OnlyKey(key))
            .await
    }
    async fn set_expiration_time(
        ...
        self.execute(command, RedisArgument::Default(key, string_u64))
            .await
    }
    pub async fn expire(&mut self, key: String, n: u64) -> Result<u8, HermesError> {
        // 设置 key 的过期时间
        // 设置成功返回1,key不存在或失败返回0
        self.set_expiration_time(RedisCommands::EXPIRE, key, n).await
    }
    pub async fn expireat(&mut self, key: String, n: u64) -> Result<u8, HermesError> {
        // 设置 key 的过期时间(Unix时间戳)
        // 设置成功返回1, key 不存在或失败返回0
        self.set_expiration_time(RedisCommands::EXPIREAT, key, n).await
    }
    pub async fn pexpire(&mut self, key: String, n: u64) -> Result<u8, HermesError> {
        // 设置 key 的过期时间亿以毫秒计
        // 设置成功返回1, key 不存在或失败返回0
        self.set_expiration_time(RedisCommands::PEXPIRE, key, n).await
    }
    pub async fn pexpireat(&mut self, key: String, n: u64) -> Result<u8, HermesError> {
        // 设置 key 的过期时间亿以毫秒计(Unix时间戳)
        // 设置成功返回1, key 不存在或失败返回0
        self.set_expiration_time(RedisCommands::PEXPIREAT, key, n).await
    }
    pub async fn keys(&mut self, pattern: RedisPattern) -> Result<Vec<String>, HermesError> {
        let patterns = match pattern {
            RedisPattern::All => vec!["*".to_string()],
            RedisPattern::Keys(keys) => keys,
            RedisPattern::StartFrom(key_start) => vec![format!("{}*", key_start)],
        };
        self.execute(RedisCommands::KEYS, RedisArgument::Items(patterns)).await
    }
