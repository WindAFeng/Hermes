...
**Hermes** 是一款专为现代应用设计的数据库网关中间件，支持通过 **Socket** 和 **WebSocket** 协议接收外部请求，并以 **JSON 指令格式** 对接多种主流数据库（MySQL、PostgreSQL、Redis、MongoDB），为操作数据库提供统一的数据操作接口。项目同时内置了智能缓存机制、优先级写入队列和高并发处理能力，适用于需要高效数据交互与统一管理的微服务架构或后端系统。
...
- ✅ **数据库支持**：
    ...
    - MongoDB(未实现)
- ✅ **智能缓存层(未实现)**：
    - 查询结果自动缓存（默认 TTL = 5 分钟，可配置）
    - 写入时自动更新或失效相关缓存
    - 支持按“重要程度”设定写入优先级，动态调整写入队列顺序
- ✅ **高并发 & 异步**：基于 `Tokio` 运行时 + `Actix-web`，充分利用 Rust 的异步与零成本抽象优势
- ✅ **Web 控制台(未实现)**：配套前端（Node.js + Vue.js + Naive UI）提供可视化终端，用于调试与监控
